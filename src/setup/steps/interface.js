...
const lockedTip = (peer) => {
	...
	if (peer.standby) {
		return `${holder ? `Held by <strong>${holder}</strong>` : 'Held by another node'}. All nodes share one virtual IP; it can be moved here from the dashboard once setup is finished.`;
	}
