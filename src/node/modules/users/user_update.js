import userModalPartial from 'node/modules/users/partials/modals/user_update.html';
...
const render = (event) => {
	...
	form.querySelector('.role').checked = user.groups.includes('admins');
	if (user.username !== 'voyager') {
		form.querySelector('.role').classList.remove('d-none');
	}
};

const restore = (event) => {
	user = null;
	form.querySelector('.role').classList.add('d-none');
	form.reset();
};
